...
bool check(char *word)
{
    ...
    // Looping character by character the word
    for (int i = 0; word[i] != '\0'; i++)
    {
        int pos = tolower(word[i]) - 'a';
        if (tempRoot->children[pos] == NULL)
        {
            return false;
        }
        tempRoot = tempRoot->children[pos];
    }
    if (tempRoot != NULL && tempRoot->is_word == true)
    {
        return true;
    }
    return false;
